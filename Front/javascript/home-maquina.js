...
const getLaptops = () => {
    ...
    asideOpcion()
    $.ajax({
        url: "http://144.22.242.102/api/laptop/all",
        type:"GET",
        contentType:'json',
        success: function(datos){
            if(datos.length == 0){
...
const setInputs = (id, opc) => {
    ...
    switch(opc){
        case 1:
            url="http://144.22.242.102/api/user/"+id; break;
        case 2:
            url="http://144.22.242.102/api/laptop/"+id; break;
    }
...
const PutUser = async () => {
    ...
    verifyEmail().done(function(datos){
        if(datos & email != $("#email").val()){
            ...
            vacios(myData)
            $.ajax({
                url: "http://144.22.242.102/api/user/update",
                type: "PUT",
